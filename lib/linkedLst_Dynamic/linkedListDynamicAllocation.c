...
void swapElementOfLinkedLst (linkedLst_t** linkedLst, int x, int y){
    linkedLst_t *buffNode = NULL;
    linkedLst_t *previousNode = NULL;
    linkedLst_t *minIndexNode = NULL;
    linkedLst_t *maxIndexNode = NULL;
    linkedLst_t *minIndexPreviousNode = NULL;
    linkedLst_t *maxIndexPreviousNode = NULL;
    // linkedLst_t *maxIndexNode = NULL;
    linkedLst_t *headNode = *linkedLst;
    linkedLst_t *tempNode = *linkedLst;
    int maxPosition = 0;
    int minPosition = 0;
    int loop = 0;
    int sizeOfLinkedLst = getSizeOfLinkedLst(tempNode);
    // index to element out of range
    if (sizeOfLinkedLst - 1 < x || sizeOfLinkedLst - 1 < y){
        return;
    }
    ...
    // find current node and previous node follow index x/y
    while (tempNode !=null && loop < maxPosition){
        if (loop == minPosition){
            if (previousNode != NULL){
                minIndexPreviousNode = previousNode;
            }
            minIndexNode = tempNode;
        }

        if (loop == maxPosition){
            maxIndexNode = tempNode;
            maxIndexPreviousNode = previousNode;
        }
    ...
    //swap node
    buffNode = maxIndexNode->pNext;
    if (minIndexPreviousNode != NULL){
        minIndexPreviousNode->pNext = maxIndexNode;
        maxIndexNode ->pNext = minIndexNode->pNext;
        minIndexNode->pNext = buffNode;
        maxIndexPreviousNode->pNext = minIndexNode;
    }
    else{
        maxIndexNode->pNext = minIndexNode->pNext;
        minIndexNode->pNext = buffNode;
...
* @ para: linkedLst_t* linkedLst
* @ para: int index
* @ return: element that get from List. return NULL mean not found element
*/
linkedLst_t* getElementOfLinkedLstUsingIndex (linkedLst_t* linkedLst, int index){
    int sizeOfLinkedLst = getSizeOfLinkedLst(linkedLst);
    // check for input
    if (index > sizeOfLinkedLst - 1){
        return NULL;
    }
    if (linkedLst == NULL){
        return NULL;
